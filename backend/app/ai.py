...
_SUMMARY_SYSTEM = (
    "你是专业的视频内容分析助手。用户会给你一段带时间戳的视频字幕，格式为 [时间] 文本。"
    "请基于字幕内容，输出一份结构化中文总结，帮助用户快速掌握视频要点。"
    "必须严格返回 JSON 对象，不要输出多余文字。JSON 结构如下：\n"
    "{\n"
    '  "overview": "用 4-6 句话写成一段完整摘要，说明视频主题、背景、核心结论、关键过程和适用场景",\n'
    '  "key_points": ["核心要点1：用一句话概括观点，并补充该观点的解释、原因或字幕依据"],\n'
    '  "outline": [{"time": "12:34", "title": "该章节讲了什么", "points": ["该章节的具体要点：用 2-3 句话细致解释，包括原因、依据、例子、影响或行动建议"]}],\n'
    '  "mindmap_markdown": "# 视频主题\\n## 分支1\\n- 子要点"\n'
    "}\n"
    "要求："
    "1. 内容忠于字幕，不准编造字幕没有的信息；"
    "2. overview 不要太短，避免只写一句泛泛而谈的结论；"
    "3. key_points 输出 6-10 条，按重要性排序，每条尽量包含具体信息、原因、例子、数据或行动建议；"
    "4. outline 输出 4-8 个章节，每个章节必须包含 2-4 条 points，形成适合阅读的层级大纲；"
    "5. 每条 points 都必须是细致解释，不能只写短标题或关键词；应包含“发生了什么 + 为什么重要 + 字幕中的依据/例子/建议”中的至少两类信息；"
    "6. outline.time 必须来自字幕中真实存在的时间点；"
    "7. 语言清晰专业，适合用户直接阅读和复盘。"
)

_FAST_SUMMARY_SYSTEM = (
    "你是视频内容快速总结助手。用户会给你带时间戳的视频字幕。"
    "请优先速度，严格返回 JSON 对象，不要输出多余文字。JSON 结构如下：\n"
    "{\n"
    '  "overview": "用 2-3 句话快速概括视频主题和核心价值",\n'
    '  "outline": [{"time": "12:34", "title": "章节标题"}]\n'
    "}\n"
    "要求：outline 输出 4-8 个章节标题即可，不要写 points，不要写思维导图；"
    "outline.time 必须来自字幕中真实存在的时间点；内容忠于字幕，不准编造。"
    "最后用1-2句话给出整体评价或一句话总结"
)

_DETAIL_SUMMARY_SYSTEM = (
    "你是专业的视频内容分析助手。用户会给你视频字幕和第一阶段快速摘要。"
    "请在快速摘要基础上补全详细解释和思维导图，严格返回 JSON 对象，不要输出多余文字。JSON 结构如下：\n"
    "{\n"
    '  "overview": "保留并适度润色为 4-6 句话的完整摘要",\n'
    '  "key_points": ["核心要点：包含解释、原因或字幕依据"],\n'
    '  "outline": [{"time": "12:34", "title": "章节标题", "points": ["用 2-3 句话细致解释该点"]}],\n'
    '  "mindmap_markdown": "# 视频主题\\n## 分支1\\n- 子要点"\n'
    "}\n"
    "要求：outline 尽量沿用第一阶段的章节；每个章节必须包含 2-4 条 points；"
    "每条 points 都要解释说明，不能只写短标题或关键词；内容忠于字幕。"
)
...
def _build_outline_explanation(
    ...
    source = source or "字幕中提到了该部分的相关背景、处理过程和主要结论。"
    return (
        f"{clean_title}：这一部分需要重点理解的是它在视频整体内容中的作用。"
        f"{source} 因此阅读时不只看章节标题，还要关注它解释了什么问题、为什么重要，以及后续可以如何应用。"
    )
